...
void fpu::set_x87_ctrl(uint16_t val) { asm volatile("fldcw %[_m]" ::[_m] "m"(val) : "memory"); }
...
static uint8_t amd_has_3dnow_ext(void) { return AMD_FPU_EDX_FEAT_QUERY(amd3dnowext); }
...
static uint8_t amd_has_prefetch(void) { return AMD_FPU_ECX_FEAT_QUERY(prefetch3d); }
static uint8_t amd_has_misal_sse(void) { return AMD_FPU_ECX_FEAT_QUERY(misalignsse); }
