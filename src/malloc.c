...
static int my_errno;
int *__errno_location(void) { return &my_errno; }
    ...
    for (p = last(b); p != b; p = p->bk) {
      ...
      if (i >= 2) {
        ...
        if ((CHUNK_SIZE_T)size >= (CHUNK_SIZE_T)(FIRST_SORTED_BIN_SIZE)) {
          assert(p->bk == b ||
                 (CHUNK_SIZE_T)chunksize(p->bk) >= (CHUNK_SIZE_T)chunksize(p));
        }
      }
...
#endif
{
    ...
    if ((CHUNK_SIZE_T)(size) > (CHUNK_SIZE_T)(nb)) {
      ...
      if (mm != (char *)(MORECORE_FAILURE)) {
        ...
        */
        ...
        if (++av->n_mmaps > av->max_n_mmaps) av->max_n_mmaps = av->n_mmaps;
  ...
  */
  ...
  if (size > 0) brk = (char *)(size_t)(MORECORE(size));
...
#endif
{
  ...
  */
    ...
    if (cp != (char *)MORECORE_FAILURE) {
      ...
      if (sum > (CHUNK_SIZE_T)(av->max_mmapped_mem)) av->max_mmapped_mem = sum;
      sum += av->sbrked_mem;
      if (sum > (CHUNK_SIZE_T)(av->max_total_mem)) av->max_total_mem = sum;
 ...
 */
  ...
  for (i = 0; i != n_elements; ++i) check_inuse_chunk(mem2chunk(marray[i]));
#endif
      ...
      if (to_reserve > 0) {
        ...
        }
        {
          ...
          do {
            ...
            /* A null pointer signals (hopefully) a race condition with another
             * thread. */
          ...
          if (contiguous) {
            ...
            to_commit = (char *)g_last->top_allocated + allocate_size -
                        (char *)g_last->top_committed;
            /* Round size to commit */
    ...
    }
    {
      ...
      if (to_decommit >= g_my_pagesize) {
        ...
        assert(0 < decommit_size && decommit_size % g_pagesize == 0);
        {
          /* Decommit this */
          int rc =
              VirtualFree((char *)base_committed, decommit_size, MEM_DECOMMIT);
          /* Check returned code for consistency */
          if (!rc) goto sbrk_exit;
#ifdef TRACE
