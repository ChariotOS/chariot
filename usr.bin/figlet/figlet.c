...
/*---------------------------------------------------------------------------
  ...
  DEFAULTFONTFILE is the filename of the font to be used if no other
    ...
    DEFAULTFONTDIR.
---------------------------------------------------------------------------*/
#define DEFAULTFONTDIR "/usr/res/figlet"
...
char *myfgets(line, maxlen, fp) char *line;
int maxlen;
FILE *fp;
{
  int c = 0;
...
void printusage(out) FILE *out;
{
  ...
  fprintf(out, "              [ -f fontfile ] [ -m smushmode ] [ -w outputwidth ]\n");
...
****************************************************************************/
void readmagic(fp, magic) FILE *fp;
char *magic;
{
  int i;
...
FILE *FIGopen(name, suffix) char *name;
char *suffix;
{
  char *fontpath;
  FILE *fontfile;
  ...
  fontpath = (char *)malloc(sizeof(char) * (namelen + MYSTRLEN(name) + MYSTRLEN(suffix) + 2));
  fontfile = NULL;
...
void readfontchar(file, theord) FILE *file;
inchr theord;
{
  int row, k;
...
void readfont() {
  ...
  numsread = sscanf(fileline, "%*c%c %d %*d %d %d %d %d %d", &hardblank, &charheight, &maxlen, &smush, &cmtlines,
                    &ffright2left, &smush2);
  ...
  while (myfgets(fileline, maxlen + 1, fontfile) == NULL ? 0 : sscanf(fileline, "%li", &theord) == 1) {
    readfontchar(fontfile, theord);
  }
  fclose(fontfile);
}
...
outchr smushem(lch, rch) outchr lch, rch;
{
  if (lch == ' ') return rch;
  if (rch == ' ') return lch;
...
int smushamt() {
  ...
  for (row = 0; row < charheight; row++) {
    if (right2left) {
      ...
      for (linebd = STRLEN(outputline[row]); ch1 = outputline[row][linebd], (linebd > 0 && (!ch1 || ch1 == ' '));
           linebd--)
        ;
...
inchr getinchr() {
  ...
  switch (multibyte) {
    ...
    case 2: /* UTF-8 */
      ...
      if (ch < 0xC0 || ch > 0xFD) return 0x0080; /* illegal first character */
      ch2 = Agetchar() & 0x3F;
      if (ch < 0xE0) return ((ch & 0x1F) << 6) + ch2;
      ch3 = Agetchar() & 0x3F;
      if (ch < 0xF0) return ((ch & 0x0F) << 12) + (ch2 << 6) + ch3;
      ch4 = Agetchar() & 0x3F;
      if (ch < 0xF8) return ((ch & 0x07) << 18) + (ch2 << 12) + (ch3 << 6) + ch4;
      ch5 = Agetchar() & 0x3F;
      if (ch < 0xFC) return ((ch & 0x03) << 24) + (ch2 << 18) + (ch3 << 12) + (ch4 << 6) + ch5;
      ch6 = Agetchar() & 0x3F;
      return ((ch & 0x01) << 30) + (ch2 << 24) + (ch3 << 18) + (ch4 << 12) + (ch5 << 6) + ch6;
    case 3: /* HZ */
      ch = Agetchar();
      if (ch == EOF) return ch;
      if (hzmode) {
