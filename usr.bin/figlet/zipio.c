...
#define RUNTIMECHECK \
  if (!ZS || (ZS->runtimetypeid1 != ZIPIOSTATETYPE) || (ZS->runtimetypeid2 != ZIPIOSTATETYPE)) return -1;
#else
#define RUNTIMEINIT
#define RUNTIMECHECK
#endif
...
#define GETUINT2(ptr, i2) \
  { i2 = (((unsigned int)*(((unsigned char *)(ptr)) + 0))) | (((unsigned int)*(((unsigned char *)(ptr)) + 1)) << 8); }
...
static void BufferInitialize(struct ZipioState *zs, int doinflate) {
  ...
  if (!zs->tmpfil) {
    if (zs->usiz >= BUFFERTHRESHOLD) zs->tmpfil = tmpfile();
  }
  ...
  if (!zs->tmpfil) {
    ...
    for (i = 0; i < PTRBUFSIZE; i++) zs->ptrbuf[i] = NULL;
  }
}
...
static int BufferRead(struct ZipioState *zs, long offset, unsigned char *buffer, long length) {
  /*
...
#else
      for (i = 0; i < len; i++) tmpbuf[i] = ptr[i];
#endif
...
#else
      for (i = 0; i < len; i++) ptr[i] = tmpbuf[i];
#endif
...
static void *inflate_malloc(long length) { return malloc((size_t)length); }

static void inflate_free(void *buffer) { free(buffer); }
...
ZFILE *Zopen(const char *path, const char *mode) {
   ...
   */
  if ((zs->sign != ZIPSIGNATURE) || (zs->flag & 1) || (zs->comp != 8) || (inplen <= 30 + zs->flen + zs->elen)) {
    /* Initialize buffering */
    ...
    zs->inflatestate = InflateInitialize((void *)zs, inflate_putbuffer, inflate_malloc, inflate_free);

    if (InflatePutBuffer(zs->inflatestate, zs->inpbuf + 30 + zs->flen + zs->elen, inplen - 30 - zs->flen - zs->elen))
...
size_t Zread(void *ptr, size_t size, size_t n, ZFILE *stream) {
  ...
  if (ZS->fileposition + length > ZS->usiz) length = ZS->usiz - ZS->fileposition;
  ...
  /* Read from the buffer */
  if (BufferRead(ZS, ZS->fileposition, (unsigned char *)ptr, length)) return 0;
...
int Zclose(ZFILE *stream) {
  ...
  if (ZS->inflatestate) {
    if (InflateTerminate(ZS->inflatestate)) ZS->errorencountered = TRUE;

    /* Check that the CRC is OK */
    if (ZS->filecrc != (ZS->crc3 ^ 0xffffffffL)) ZS->errorencountered = TRUE;
  }
