...
class typed_transfer {
 public:
  static void move(T* destination, T* source, size_t count) {
    ...
    for (size_t i = 0; i < count; ++i) new (&destination[i]) T(move(source[i]));
  }
  ...
  static void copy(T* destination, const T* source, size_t count) {
    ...
    for (size_t i = 0; i < count; ++i) new (&destination[i]) T(source[i]);
  }
...
class vec {
 ...
 public:
  ...
  void clear_with_capacity() {
    for (int i = 0; i < m_size; ++i) data()[i].~T();
    m_size = 0;
  }
  ...
  void shrink(int new_size) {
    ...
    for (int i = new_size; i < size(); ++i) at(i).~T();
    m_size = new_size;
  }
  ...
  void resize(int new_size) {
    ...
    for (int i = size(); i < new_size; ++i) new (slot(i)) T;
    m_size = new_size;
  }
 ...
 private:
  void merge(int l, int m, int r) {
    ...
    for (i = 0; i < n1; i++) L[i] = buf[l + i];
    for (j = 0; j < n2; j++) R[j] = buf[m + 1 + j];
