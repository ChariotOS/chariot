...
gfx::isize ui::boxlayout::preferred_size(ui::view& view) const {
  gfx::isize size;
  size.set_primary_size_for_direction(direction(), preferred_primary_size(view));
  size.set_secondary_size_for_direction(direction(), preferred_secondary_size(view));
  return size;
}
  ...
  for (auto& child : view.m_children) {
    ...
    if (child->is_shrink_to_fit() && child->layout()) {
      preferred_primary_size =
          child->layout()->preferred_size(*child).primary_size_for_direction(direction());
    }
  ...
  for (auto& entry : view.m_children) {
    ...
    if (entry->is_shrink_to_fit() && entry->layout()) {
      preferred_secondary_size =
          entry->layout()->preferred_size(*entry).secondary_size_for_direction(direction());
...
void ui::boxlayout::run(ui::view& view) {
  ...
  for (size_t i = 0; i < ents.size(); i++) {
    ...
    if (entry->is_shrink_to_fit() && entry->layout()) {
      auto preferred_size = entry->layout()->preferred_size(*entry);
    ...
    Item item;
    item.view = entry.get();
    item.min_size = min_size.primary_size_for_direction(direction());
    item.max_size = max_size.primary_size_for_direction(direction());

    item.min_size += item.view->padding().total_for(direction());
  ...
  while (unfinished_items && available_size > 0) {
    ...
    // if (direction() == gfx::Direction::Vertical) printf("pixels: %d\n", pixels);
  ...
  for (auto& item : items) {
    ...
    // min_secondary += item.view->padding().total_for_secondary(direction());
