    ...
    namespace impl {
      ...
      struct color {
        ...
        color blend(color source) const {
          ...
          uint8_t r =
              (red() * alpha() * (255 - source.alpha()) + 255 * source.alpha() * source.red()) / d;
          uint8_t g =
              (green() * alpha() * (255 - source.alpha()) + 255 * source.alpha() * source.green()) /
              d;
          uint8_t b =
              (blue() * alpha() * (255 - source.alpha()) + 255 * source.alpha() * source.blue()) /
              d;
          uint8_t a = d / 255;
