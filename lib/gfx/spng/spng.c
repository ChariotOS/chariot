...
#define SPNG_VERSION_STRING \
  SPNG_STR(SPNG_VERSION_MAJOR) "." SPNG_STR(SPNG_VERSION_MINOR) "." SPNG_STR(SPNG_VERSION_PATCH)
...
static struct spng__iter spng__iter_init(unsigned bit_depth, const unsigned char *samples) {
  struct spng__iter iter = {.mask = (uint16_t)(1 << bit_depth) - 1,
...
static int check_ihdr(const struct spng_ihdr *ihdr, uint32_t max_width, uint32_t max_height) {
  ...
  switch (ihdr->color_type) {
    ...
    case SPNG_COLOR_TYPE_TRUECOLOR_ALPHA: {
      if (!(ihdr->bit_depth == 8 || ihdr->bit_depth == 16)) return SPNG_EBIT_DEPTH;
...
static int read_non_idat_chunks(spng_ctx *ctx) {
  ...
  while (!(ret = read_header(ctx, &discard))) {
    ...
    if (is_small_chunk(chunk.type)) { /* The largest of these chunks is PLTE with 256 entries */
    ...
    } else /* Arbitrary-length chunk */
    {
      if (!memcmp(chunk.type, type_exif, 4)) {
        ...
        ctx->stored.iccp = 1;
      } else if (!memcmp(chunk.type, type_text, 4) || !memcmp(chunk.type, type_ztxt, 4) ||
        ...
        if (!memcmp(chunk.type, type_text, 4)) {
          ...
          const unsigned char *zlib_stream =
              memchr(data + translated_keyword_offset, 0, peek_bytes - translated_keyword_offset);
