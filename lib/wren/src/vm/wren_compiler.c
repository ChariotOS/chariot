...
static void printError(Parser* parser, int line, const char* label, const char* format,
                       va_list args) {
...
static Keyword keywords[] = {
    {"break", 5, TOKEN_BREAK},     {"class", 5, TOKEN_CLASS},   {"construct", 9, TOKEN_CONSTRUCT},
    {"else", 4, TOKEN_ELSE},       {"false", 5, TOKEN_FALSE},   {"for", 3, TOKEN_FOR},
    {"foreign", 7, TOKEN_FOREIGN}, {"if", 2, TOKEN_IF},         {"import", 6, TOKEN_IMPORT},
    {"in", 2, TOKEN_IN},           {"is", 2, TOKEN_IS},         {"null", 4, TOKEN_NULL},
    {"return", 6, TOKEN_RETURN},   {"static", 6, TOKEN_STATIC}, {"super", 5, TOKEN_SUPER},
    {"this", 4, TOKEN_THIS},       {"true", 4, TOKEN_TRUE},     {"var", 3, TOKEN_VAR},
...
static int declareVariable(Compiler* compiler, Token* token) {
  ...
  if (compiler->scopeDepth == -1) {
    int line = -1;
    int symbol = wrenDefineVariable(compiler->parser->vm, compiler->parser->module, token->start,
                                    token->length, NULL_VAL, &line);
...
static int methodSymbol(Compiler* compiler, const char* name, int length) {
  return wrenSymbolTableEnsure(compiler->parser->vm, &compiler->parser->vm->methodNames, name,
                               length);
}

// Appends characters to [name] (and updates [length]) for [numParams] "_"
// surrounded by [leftBracket] and [rightBracket].
static void signatureParameterList(char name[MAX_METHOD_SIGNATURE], int* length, int numParams,
...
static void field(Compiler* compiler, bool canAssign) {
  ...
  if (enclosingClass == NULL) {
    ...
    field =
        wrenSymbolTableEnsure(compiler->parser->vm, &enclosingClass->fields,
...
static void name(Compiler* compiler, bool canAssign) {
  ...
  variable.scope = SCOPE_MODULE;
  variable.index =
      wrenSymbolTableFind(&compiler->parser->module->variableNames, token->start, token->length);
  if (variable.index == -1) {
    // Implicitly define a module-level variable in
    // the hopes that we get a real definition later.
    variable.index = wrenDeclareVariable(compiler->parser->vm, compiler->parser->module,
...
static void forStatement(Compiler* compiler) {
  ...
  if (compiler->numLocals + 2 > MAX_LOCALS) {
    error(compiler,
          "Cannot declare more than %d variables in one scope. (Not enough space for for-loops "
          "internal variables)",
          MAX_LOCALS);
    return;
  }
...
static void import(Compiler* compiler) {
  ...
  do {
    ...
    int variableConstant = addConstant(
        compiler, wrenNewStringLength(compiler->parser->vm, compiler->parser->previous.start,
