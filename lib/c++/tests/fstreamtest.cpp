...
int main() {
  ...
  std::cout << "Now reading other input file to see what wonderful goodness we can discover"
            << std::endl;
  ...
  if (!inFile.is_open()) {
    ...
    std::cout << "This test wasn't designed to run without the uClibc++ driver behind it"
              << std::endl;
    return 1;
  }
  inFile.seekg(27);
  ...
  if (correctValue(27) != b) {
    std::cout << "Read in invalid value.  Read in " << (int)b << ", expected "
              << (int)correctValue(27) << std::endl;
  } else {
