...
template <class T>
void testClass(std::string tname);
...
void testClass(std::string tname) {
  ...
  if (std::char_traits<T>::compare("test", "test", 3) == 0) {
    ...
    std::cout << "compare() does not properly compare partial identical strings for " << tname
              << std::endl;
  }

  if (std::char_traits<T>::compare("test", "test", 6) == 0) {
    std::cout << "compare() properly compares undersized identical strings for " << tname
              << std::endl;
  } else {
    std::cout << "compare() does not properly compare undersized identical strings for " << tname
              << std::endl;
  }

  if (std::char_traits<T>::compare("test", "testing", 4) == 0) {
    std::cout << "compare() properly compares different sized identical strings for " << tname
              << std::endl;
  } else {
    std::cout << "compare() does not properly compare different sized identical strings for "
              << tname << std::endl;
  }
  ...
  if (std::char_traits<T>::find(a1, sizeof(a1), 't') == a1) {
    std::cout << "find() properly locates the location of 't' starting at the beginning for "
              << tname << std::endl;
  } else {
    std::cout
        << "find() does not properly locate the location of 't' starting at the beginning for "
        << tname << std::endl;
  }

  if (std::char_traits<T>::find(a1 + 1, sizeof(a1) - 1, 't') == (a1 + 3)) {
    std::cout << "find() properly locates the location of 't' with offset 1 for " << tname
              << std::endl;
  } else {
    std::cout
        << "find() does not properly locate the location of 't' starting at the beginning for "
  ...
  if (std::char_traits<T>::find(a1 + 3, sizeof(a1) - 3, 't') == (a1 + 3)) {
    std::cout << "find() properly locates the location of 't' with offset 3 for " << tname
              << std::endl;
  } else {
    std::cout
        << "find() does not properly locate the location of 't' starting at the beginning for "
  ...
  if (std::char_traits<T>::find(a1, 0, 't') == 0) {
    ...
    std::cout << "find() does not properly return NULL in 0-sized searches for " << tname
              << std::endl;
  }
