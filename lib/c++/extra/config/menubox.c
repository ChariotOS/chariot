 ...
 */
int dialog_menu(const char *title, const char *prompt, int height, int width, int menu_height,
                const char *current, int item_no, struct dialog_list_item **items) {
  ...
  draw_box(dialog, box_y, box_x, menu_height + 2, menu_width + 2, menubox_border_attr,
           menubox_attr);
  ...
  if ((f = fopen("lxdialog.scrltmp", "r")) != NULL) {
    if ((fscanf(f, "%d\n", &scroll) == 1) && (scroll <= choice) && (scroll + max_choice > choice) &&
        (scroll >= 0) && (scroll + max_choice <= item_no)) {
      first_item = scroll;
  ...
  print_arrows(dialog, item_no, scroll, box_y, box_x + item_x + 1, menu_height);
  ...
  while (key != ESC) {
    ...
    if (strchr("ynm", key))
      ...
      for (i = choice + 1; i < max_choice; i++) {
        ...
        if (key == tolower(items[scroll + i]->name[j])) break;
      }
      if (i == max_choice)
        for (i = 0; i < max_choice; i++) {
          j = first_alpha(items[scroll + i]->name, "YyNnMm>");
    ...
    if (i < max_choice || key == KEY_UP || key == KEY_DOWN || key == '-' || key == '+' ||
      ...
      if (key == KEY_UP || key == '-') {
        if (choice < 2 && scroll) {
          ...
          print_item(menu, items[scroll]->name, 0, FALSE, (items[scroll]->tag[0] != ':'));
        } else
        ...
        if ((choice > max_choice - 3) && (scroll + max_choice < item_no)) {
          /* Scroll menu up */
          scrollok(menu, TRUE);
          scroll(menu);
          scrollok(menu, FALSE);
          ...
          print_item(menu, items[scroll + max_choice - 1]->name, max_choice - 1, FALSE,
    ...
    switch (key) {
      ...
      case TAB:
      case KEY_RIGHT:
        button = ((key == KEY_LEFT ? --button : ++button) < 0) ? 2 : (button > 2 ? 0 : button);
