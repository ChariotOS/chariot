...
void sym_calc_value(struct symbol *sym) {
    ...
    case S_TRISTATE:
      if (sym_is_choice_value(sym) && sym->visible == yes) {
        ...
        if (sym_has_value(sym))
          ...
          if (prop) newval.tri = expr_calc_value(prop->expr);
        }
      ...
      if (sym_get_type(sym) == S_BOOLEAN) {
        ...
        if (sym->visible == mod) sym->visible = yes;
        if (sym->rev_dep.tri == mod) sym->rev_dep.tri = yes;
      }
      break;
    case S_STRING:
    case S_HEX:
  ...
  if (memcmp(&oldval, &sym->curr, sizeof(oldval))) sym_set_changed(sym);
  ...
  if (sym_is_choice(sym)) {
    ...
    for (e = prop->expr; e; e = e->left.expr) {
      ...
      if (flags & SYMBOL_CHANGED) sym_set_changed(e->right.sym);
    }
  }
}
...
bool sym_set_tristate_value(struct symbol *sym, tristate val) {
  ...
  if (oldval != val) {
    sym_clear_all_valid();
    if (sym == modules_sym) sym_set_all_changed();
  }
...
struct symbol *sym_check_deps(struct symbol *sym) {
  ...
  if (sym->flags & SYMBOL_CHECK_DONE) return NULL;
  ...
  for (prop = sym->prop; prop; prop = prop->next) {
    if (prop->type == P_CHOICE) continue;
...
struct symbol *prop_get_symbol(struct property *prop) {
  if (prop->expr && (prop->expr->type == E_SYMBOL || prop->expr->type == E_CHOICE))
