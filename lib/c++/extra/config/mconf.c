...
static int cprint_name(const char *fmt, ...) {
  ...
  if (res > 0) items[item_no - 1]->namelen += res;
  va_end(ap);
...
static void build_conf(struct menu *menu) {
    ...
    if (prop && menu != current_menu) {
      ...
      switch (prop->type) {
        case P_MENU:
          ...
          if (single_menu_mode) {
            cprint_name("%s%*c%s", menu->data ? "-->" : "++>", indent + 1, ' ', prompt);
          } else {
            cprint_name("   %*c%s  --->", indent + 1, ' ', prompt);
          }

          if (single_menu_mode && menu->data) goto conf_childs;
          return;
        default:
          if (prompt) {
            child_count++;
  ...
  if (sym_is_choice(sym)) {
    ...
    for (child = menu->list; child; child = child->next) {
      if (menu_is_visible(child) && child->sym == def_sym) def_menu = child;
    }
      ...
      switch (type) {
        ...
        default:
          ...
          cprint_name("%*c%s%s", tmp, ' ', menu_get_prompt(menu),
                      (sym_has_value(sym) || !sym_is_changable(sym)) ? "" : " (NEW)");
          goto conf_childs;
      }
    }
...
static void conf(struct menu *menu) {
  ...
  while (1) {
    ...
    dialog_clear();
    stat = dialog_menu(prompt ? prompt : "Main Menu", menu_instructions, rows, cols, rows - 10,
                       active_entry, item_no, items);
    if (stat < 0) return;

    if (stat == 1 || stat == 255) break;
    ...
    sym = NULL;
    submenu = NULL;
    if (sscanf(active_entry + 1, "%p", &submenu) == 1) sym = submenu->sym;
    ...
    switch (stat) {
      ...
      case 3:
        if (type == 't') {
          if (sym_set_tristate_value(sym, yes)) break;
          if (sym_set_tristate_value(sym, mod)) show_textbox(NULL, setmod_text, 6, 74);
        }
        break;
      case 4:
        if (type == 't') sym_set_tristate_value(sym, no);
        break;
      case 5:
        if (type == 't') sym_set_tristate_value(sym, mod);
        break;
      case 6:
        if (type == 't')
...
static void conf_choice(struct menu *menu) {
  ...
  while (1) {
    ...
    cinit();
    for (child = menu->list; child; child = child->next) {
      if (!menu_is_visible(child)) continue;
      cmake();
      cprint_tag("%p", child);
    ...
    switch (dialog_checklist(prompt ? prompt : "Main Menu", radiolist_instructions, 15, 70, 6,
                             item_no, items, FLAG_RADIO)) {
      case 0:
        if (sscanf(first_sel_item(item_no, items)->tag, "%p", &menu) != 1) break;
...
static void conf_string(struct menu *menu) {
  ...
  while (1) {
    ...
    switch (dialog_inputbox(prompt ? prompt : "Main Menu", heading, 10, 75,
                            sym_get_string_value(menu->sym))) {
      case 0:
        if (sym_set_string_value(menu->sym, dialog_input_result)) return;
...
static void conf_load(void) {
  while (1) {
    switch (dialog_inputbox(NULL, load_config_text, 11, 55, filename)) {
      case 0:
        if (!dialog_input_result[0]) return;
        if (!conf_read(dialog_input_result)) return;
...
static void conf_save(void) {
  while (1) {
    switch (dialog_inputbox(NULL, save_config_text, 11, 55, filename)) {
      case 0:
        if (!dialog_input_result[0]) return;
        if (!conf_write(dialog_input_result)) return;
...
int main(int ac, char **av) {
  ...
  sym_calc_value(sym);
  snprintf(menu_backtitle, 128, "uClibc v%s Configuration", sym_get_string_value(sym));

  mode = getenv("MENUCONFIG_MODE");
  if (mode) {
    if (!strcasecmp(mode, "single_menu")) single_menu_mode = 1;
  }
  ...
  init_dialog();
  do {
    stat = dialog_yesno(NULL, "Do you wish to save your new uClibc configuration?", 5, 60);
  } while (stat < 0);
  end_dialog();
