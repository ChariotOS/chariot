...
struct expr *menu_check_dep(struct expr *e) {
  ...
  switch (e->type) {
    ...
    case E_SYMBOL:
      ...
      if (e->left.sym == &symbol_mod) return expr_alloc_and(e, expr_alloc_symbol(modules_sym));
      break;
    default:
      break;
  }
  ...
  if (parent->list) {
    ...
    for (menu = parent->list; menu; menu = menu->next) {
      ...
      for (; prop; prop = prop->next) {
        if (prop->menu != menu) continue;
        dep = expr_transform(prop->visible.expr);
        dep = expr_alloc_and(expr_copy(basedep), dep);
        dep = expr_eliminate_dups(dep);
        if (menu->sym && menu->sym->type != S_TRISTATE) dep = expr_trans_bool(dep);
        prop->visible.expr = dep;
        if (prop->type == P_SELECT) {
          struct symbol *es = prop_get_symbol(prop);
          es->rev_dep.expr = expr_alloc_or(
              es->rev_dep.expr, expr_alloc_and(expr_alloc_symbol(menu->sym), expr_copy(dep)));
        }
      }
    }
    for (menu = parent->list; menu; menu = menu->next)
    ...
    for (menu = parent->next; menu; menu = menu->next) {
      ...
      if (expr_depends_symbol(dep, sym)) goto next;
...
bool menu_is_visible(struct menu *menu) {
  ...
  if (visible != no) return true;
  if (!sym || sym_get_tristate_value(menu->sym) == no) return false;
