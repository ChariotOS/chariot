...
static const unsigned char *lc_names[] = {
    "LC_CTYPE", "LC_NUMERIC", "LC_MONETARY", "LC_TIME",      "LC_COLLATE",     "LC_MESSAGES",
...
static void do_locale_names(void) {
  ...
  if (num_locales <= 1) {
    ...
    if (at_strings_end > at_strings) {
      ...
      p = at_strings;
      while (*p) {
        fprintf(ofp, "\n\t%4d, '%c',", (unsigned char)*p, /* len of string\0 */
                at_mapto[i]);
...
static int find_codeset_num(const char *cs) {
  ...
  if (strcmp(cs, "UTF-8") != 0) {
    ...
    while (*s && strcmp(__LOCALE_DATA_CODESET_LIST + ((unsigned char)*s), cs)) {
      /*  			printf("tried %s\n", __LOCALE_DATA_CODESET_LIST + ((unsigned char)
       * *s)); */
      ++r;
      ++s;
    }
...
static int le_cmp(const void *a, const void *b) {
  ...
  if (!(r = p->name[0] - q->name[0]) && !(r = p->name[1] - q->name[1]) &&
      !(r = p->name[3] - q->name[3]) && !(r = p->name[4] - q->name[4]) &&
      !(r = p->name[2] - q->name[2]) && !(r = -(p->cs - q->cs))) {
    r = -(p->dot_cs - q->dot_cs);
...
#define DO_LC_COMMON(CATEGORY)                                                                     \
  ...
  printf("total = %d + %d * %d + %d = %d\n", num_locales, lc_##CATEGORY##_uniq, NUM_NL_##CATEGORY, \
         (int)(last - buf),                                                                        \
...
#define DL_LC_LOOPTAIL(CATEGORY)                                                           \
  ...
  {                                                                                        \
    ...
    for (r = 0; r < lc_##CATEGORY##_uniq; r++) {                                           \
      if (!memcmp(lc_##CATEGORY##_uniq_X[lc_##CATEGORY##_uniq], lc_##CATEGORY##_uniq_X[r], \
                  NUM_NL_##CATEGORY)) {                                                    \
        break;                                                                             \
      }                                                                                    \
    }                                                                                      \
...
#define __C_isdigit(c)                                               \
  ((sizeof(c) == sizeof(char)) ? (((unsigned char)((c) - '0')) < 10) \
                               : (((unsigned int)((c) - '0')) < 10))
#define __C_isalpha(c)                                                        \
  ((sizeof(c) == sizeof(char)) ? (((unsigned char)(((c) | 0x20) - 'a')) < 26) \
