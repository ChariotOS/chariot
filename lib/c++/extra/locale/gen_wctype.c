...
#define __CTYPE_isxdigit(D, X) (__CTYPE_isdigit(D) || (((unsigned int)(((X) | 0x20) - 'a')) <= 5))
...
static void dump_table_data(table_data *tbl) {
  printf(
      "ii_shift = %d  ti_shift = %d\n"
      "ii_len = %d  ti_len = %d  ut_len = %d\n"
      "total = %d\n",
      tbl->ii_shift, tbl->ti_shift, tbl->ii_len, tbl->ti_len, tbl->ut_len,
...
/*  					} */
        ...
        combt[c / 4] |=
            ((((!!iswctype(c, is_comb)) << 1) | !!iswctype(c, is_comb3)) << ((c & 3) << 1));
