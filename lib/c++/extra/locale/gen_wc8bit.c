  ...
  while (--argc) {
    ...
    memset(xi, sizeof(xi), 0);
    {
      ...
      while (fgets(buf, sizeof(buf), fp)) {
        if ((2 != sscanf(buf, "{ %lx , %lx", &c, &wc)) || (c >= 256) || (wc > MAX_WCHAR)) {
      ...
      for (i = 128; i < 256; i++) {
        ...
        if ((i & (CTYPE_ROW_LEN - 1)) == (CTYPE_ROW_LEN - 1)) {
          ...
          if (!((i >> CTYPE_IDX_SHIFT) & 0x7) && (i != (127 + CTYPE_ROW_LEN))) {
            fprintf(out, "\n\t\t ");
          }
          fprintf(out, " %#4x,", j);
        }
      ...
      for (i = 128; i < 256; i++) {
        ...
        if ((c != 0) || 1) {
           ...
           */
          /*  						) { */
          /*  						printf("error - uplow diff out of range! %d
           * %ld %ld\n",
           */
          /*  							   i, u, l); */
          /*  						return EXIT_FAILURE; */
          ...
          if ((i & (UPLOW_ROW_LEN - 1)) == (UPLOW_ROW_LEN - 1)) {
            ...
            if (!((i >> UPLOW_IDX_SHIFT) & 0x7) && (i != (127 + UPLOW_ROW_LEN))) {
              fprintf(out, "\n\t\t ");
            }
            fprintf(out, " %#4x,", j);
          }
...
/*  					fprintf(out, " %4ld,", (l==i) ? u : l); */
...
#else
            fprintf(out, " %#08lx, %8ld, %d, %8ld, %d, %#08lx\n", (unsigned long)i, (long)(l - i),
                    iswupper(c), (long)(i - u), iswlower(c), (unsigned long)c);
#endif
          }
#endif
        }
      ...
      for (i = 128; i < 256; i++) {
        ...
        if ((i & (C2WC_ROW_LEN - 1)) == (C2WC_ROW_LEN - 1)) {
          ...
          if (!((i >> C2WC_IDX_SHIFT) & 0x7) && (i != (127 + C2WC_ROW_LEN))) {
            fprintf(out, "\n\t\t ");
          }
          fprintf(out, " %#4x,", j);
        }
  ...
  fprintf(out, "\" \\\n\t\"\\0\"");
  for (i = 0; i < numsets; i++) {
    fprintf(out, " \\\n\t\"%s\\0\"", codeset_list + ((unsigned char)codeset_index[i]));
  }
  ...
  printf("size is %d * %d  +  %d * %d  + %d * %d  =  %d\n", tt_num, 1 << TT_SHIFT, ti_num,
         1 << TI_SHIFT, ((MAX_WCHAR >> (TT_SHIFT + TI_SHIFT)) + 1), numsets,
