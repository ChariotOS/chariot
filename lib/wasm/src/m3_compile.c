...
static inline pc_t GetPC(IM3Compilation o) { return GetPagePC(o->page); }
...
static M3_NOINLINE M3Result EnsureCodePageNumLines(IM3Compilation o, u32 i_numLines) {
  ...
  if (NumFreeLines(o->page) < i_numLines) {
    ...
    if (page) {
      m3log(emit, "bridging new code page from: %d %p (free slots: %d) to: %d", o->page->info.sequence, GetPC(o), NumFreeLines(o->page),
          page->info.sequence);
...
static M3_NOINLINE M3Result EmitOp(IM3Compilation o, IM3Operation i_operation) {
  ...
  if (o->page) {
    ...
    if (i_operation != op_DumpStack) o->numEmits++;
#endif
...
static const IM3Operation c_setGlobalOps[] = {NULL, op_SetGlobal_i32, op_SetGlobal_i64, FPOP(op_SetGlobal_f32), FPOP(op_SetGlobal_f64)};
...
static inline void ReleaseCompilationCodePage(IM3Compilation o) { ReleaseCodePage(o->runtime, o->page); }
...
static u8 GetStackTypeFromTop(IM3Compilation o, u16 i_offset) {
  ...
  if (o->stackIndex >= i_offset) {
    ...
    if (index >= o->stackFirstDynamicIndex) type = o->typeStack[index];
  }

  return type;
}

static inline u8 GetStackTopType(IM3Compilation o) { return GetStackTypeFromTop(o, 0); }

static inline u8 GetStackTypeFromBottom(IM3Compilation o, u16 i_offset) {
  u8 type = c_m3Type_none;

  if (i_offset < o->stackIndex) type = o->typeStack[i_offset];
...
static inline u16 GetSlotForStackIndex(IM3Compilation o, u16 i_stackIndex) {
  ...
  if (i_stackIndex < o->stackIndex) slot = o->wasmStack[i_stackIndex];
...
static inline M3Result AllocateConstantSlots(IM3Compilation o, u16 *o_slot, u8 i_type) {
  u16 maxTableIndex = o->slotFirstConstIndex + d_m3MaxConstantTableSize;
	u16 start =  o->slotFirstConstIndex;
	u16 end = M3_MIN(o->slotFirstDynamicIndex, maxTableIndex);

	// printf("range: %d..%d\n", start, end);
  return AllocateSlotsWithinRange(o, o_slot, i_type, start, end);
}
...
static inline bool IsRegisterTypeAllocated(IM3Compilation o, u8 i_type) { return IsRegisterAllocated(o, IsFpType(i_type)); }
...
u16 GetMaxUsedSlotPlusOne(IM3Compilation o) {
  while (o->slotMaxAllocatedIndexPlusOne > o->slotFirstDynamicIndex) {
    if (IsSlotAllocated(o, o->slotMaxAllocatedIndexPlusOne - 1)) break;
...
static M3Result PreserveNonTopRegisters(IM3Compilation o) {
  ...
  if (stackTop >= 0) {
    ...
    {
      if (GetRegisterStackIndex(o, 0) != stackTop) _(PreserveRegisterIfOccupied(o, c_m3Type_i64));
    }

    if (IsRegisterAllocated(o, 1))  // fp0
    {
      if (GetRegisterStackIndex(o, 1) != stackTop) _(PreserveRegisterIfOccupied(o, c_m3Type_f64));
    }
  }
...
static inline M3Result PushAllocatedSlotAndEmit(IM3Compilation o, u8 i_type) { return _PushAllocatedSlotAndEmit(o, i_type, true); }

static inline M3Result PushAllocatedSlot(IM3Compilation o, u8 i_type) { return _PushAllocatedSlotAndEmit(o, i_type, false); }
...
static M3Result ReturnValues(IM3Compilation o, IM3CompilationScope i_functionBlock, bool i_isBranch) {
  ...
  if (not IsStackPolymorphic(o)) _throwif(m3Err_typeCountMismatch, i_isBranch ? (blockHeight < numReturns) : (blockHeight != numReturns));
  ...
  if (numReturns) {
    ...
    for (u16 i = 0; i < numReturns; ++i) {
      ...
      if (IsStackPolymorphic(o) and stackType == c_m3Type_none) stackType = returnType;
...
static M3Result Compile_SetLocal(IM3Compilation o, m3opcode_t i_opcode) {
  ...
  if (localIndex < GetFunctionNumArgsAndLocals(o->function)) {
    ...
    if (i_opcode != c_waOp_teeLocal) _(Pop(o));
  } else
...
static M3Result Compile_GetLocal(IM3Compilation o, m3opcode_t i_opcode) {
  _try {
    ...
    if (localIndex >= GetFunctionNumArgsAndLocals(o->function)) _throw("local index out of bounds");
...
static M3Result Compile_Branch(IM3Compilation o, m3opcode_t i_opcode) {
  ...
  } else  // forward branch
  {
    ...
    if (i_opcode == c_waOp_branch) _(SetStackPolymorphic(o));
  }
...
static M3Result Compile_Call(IM3Compilation o, m3opcode_t i_opcode) {
  _try {
    ...
    if (function) {
      m3log(compile, d_indent " (func= [%d] '%s'; args= %d)", get_indention_string(o), functionIndex, m3_GetFunctionName(function),
          function->funcType->numArgs);
      if (function->module) {
        u16 slotTop;
        ...
        _throw(
            ErrorCompile(m3Err_functionImportMissing, o, "'%s.%s'", GetFunctionImportModuleName(function), m3_GetFunctionName(function)));
      }
    } else
...
static M3Result Compile_CallIndirect(IM3Compilation o, m3opcode_t i_opcode) {
  _try {
    ...
    if (IsStackTopInRegister(o)) _(PreserveRegisterIfOccupied(o, c_m3Type_i32));
...
static M3Result Compile_Nop(IM3Compilation o, m3opcode_t i_opcode) { return m3Err_none; }
...
static M3Result Compile_Operator(IM3Compilation o, m3opcode_t i_opcode) {
  ...
  if (op) {
    ...
    if (opInfo->stackOffset < 0) _(EmitSlotNumOfStackTopAndPop(o));

    if (opInfo->type != c_m3Type_none) _(PushRegister(o, opInfo->type));
  } else {
#ifdef DEBUG
...
static M3Result Compile_Load_Store(IM3Compilation o, m3opcode_t i_opcode) {
  _try {
    ...
    if (IsFpType(opInfo->type)) _(PreserveRegisterIfOccupied(o, c_m3Type_f64));
...
M3Result CompileBlockStatements(IM3Compilation o) {
  ...
  while (o->wasm < o->wasmEnd) {
    ...
    if (opinfo == NULL) _throw(ErrorCompile(m3Err_unknownOpcode, o, "opcode '%x' not available", opcode));
  ...
  */
  ...
  _try {
    ...
    for (u16 i = 0; i < numParams; ++i) {
      ...
      if (slot >= o->slotFirstDynamicIndex) MarkSlotsAllocatedByType(o, slot, type);
    }
    ...
    {
      if (not IsStackPolymorphic(o)) _(ResolveBlockResults(o, &o->block, /* isBranch: */ false));
...
static M3Result CompileLocals(IM3Compilation o) {
  ...
  if (o->function) o->function->numLocals = numLocals;
...
static M3Result ReserveConstants(IM3Compilation o) {
  ...
  while (wa < o->wasmEnd) {
    ...
    if (numConstantSlots >= d_m3MaxConstantTableSize) break;
  }
  ...
  if (o->slotFirstDynamicIndex >= d_m3MaxFunctionSlots) _throw(m3Err_functionStackOverflow);
...
M3Result CompileFunction(IM3Function io_function) {
  ...
  _try {
    ...
    m3log(compile, "unique constant slots: %d; unused slots: %d", numConstantSlots, o->slotFirstDynamicIndex - o->slotMaxConstIndex);
