...
#define FT_TRACE(level, varformat)                                                   \
  do {                                                                               \
    if (ft_trace_levels[FT_TRACE_COMP(FT_COMPONENT)] >= level) FT_Message varformat; \
  } while (0)
...
#define FT_THROW(e) \
  (FT_Throw(FT_ERR_CAT(FT_ERR_PREFIX, e), __LINE__, __FILE__) | FT_ERR_CAT(FT_ERR_PREFIX, e))
