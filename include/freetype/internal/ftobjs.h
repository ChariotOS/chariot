...
#define ft_isxdigit(x) \
  (((unsigned)(x) - '0') < 10U || ((unsigned)(x) - 'a') < 6U || ((unsigned)(x) - 'A') < 6U)
...
typedef FT_Int (*FT_CMap_CharVarIsDefaultFunc)(FT_CMap cmap, FT_UInt32 char_code,
                                               FT_UInt32 variant_selector);
...
#define FT_DEFINE_CMAP_CLASS(class_, size_, init_, done_, char_index_, char_next_,                 \
                             char_var_index_, char_var_default_, variant_list_, charvariant_list_, \
                             variantchar_list_)                                                    \
...
#define FT_REQUEST_HEIGHT(req)                                                       \
  ((req)->vertResolution ? ((req)->height * (FT_Pos)(req)->vertResolution + 36) / 72 \
                         : (req)->height)
...
typedef FT_Error (*FT_Face_GetGlyphNameFunc)(FT_Face face, FT_UInt glyph_index, FT_Pointer buffer,
                                             FT_UInt buffer_max);
 ...
 *   used).
 */
#define FT_DEFINE_RASTER_FUNCS(class_, glyph_format_, raster_new_, raster_reset_,  \
...
#define FT_DEFINE_RENDERER(class_, flags_, size_, name_, version_, requires_, interface_, init_,  \
                           done_, get_interface_, glyph_format_, render_glyph_, transform_glyph_, \
                           get_glyph_cbox_, set_mode_, raster_class_)                             \
  FT_CALLBACK_TABLE_DEF                                                                           \
  const FT_Renderer_Class class_ = {                                                              \
      FT_DEFINE_ROOT_MODULE(flags_, size_, name_, version_, requires_, interface_, init_, done_,  \
                            get_interface_) glyph_format_,                                        \
                                                                                                  \
      render_glyph_,                                                                              \
      transform_glyph_,                                                                           \
      get_glyph_cbox_,                                                                            \
...
#define FT_DEFINE_ROOT_MODULE(flags_, size_, name_, version_, requires_, interface_, init_, done_, \
                              get_interface_)                                                      \
  {                                                                                                \
      flags_,     size_,                                                                           \
                                                                                                   \
...
#define FT_DEFINE_MODULE(class_, flags_, size_, name_, version_, requires_, interface_, init_, \
                         done_, get_interface_)                                                \
  FT_CALLBACK_TABLE_DEF                                                                        \
  const FT_Module_Class class_ = {                                                             \
      flags_,     size_,                                                                       \
                                                                                               \
