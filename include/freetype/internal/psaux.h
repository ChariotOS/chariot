...
typedef struct PS_Parser_FuncsRec_ {
  ...
  FT_Error (*load_field_table)(PS_Parser parser, const T1_Field field, void** objects,
                               FT_UInt max_objects, FT_ULong* pflags);
...
typedef FT_Error (*CFF_Decoder_Get_Glyph_Callback)(TT_Face face, FT_UInt glyph_index,
                                                   FT_Byte** pointer, FT_ULong* length);
...
typedef struct T1_Decoder_FuncsRec_ {
  FT_Error (*init)(T1_Decoder decoder, FT_Face face, FT_Size size, FT_GlyphSlot slot,
                   FT_Byte** glyph_names, PS_Blend blend, FT_Bool hinting, FT_Render_Mode hint_mode,
...
typedef struct CFF_Builder_FuncsRec_ {
  void (*init)(CFF_Builder* builder, TT_Face face, CFF_Size size, CFF_GlyphSlot glyph,
               FT_Bool hinting);
...
typedef struct CFF_Decoder_FuncsRec_ {
  void (*init)(CFF_Decoder* decoder, TT_Face face, CFF_Size size, CFF_GlyphSlot slot,
  ...
  FT_Error (*parse_charstrings_old)(CFF_Decoder* decoder, FT_Byte* charstring_base,
...
#define IS_PS_SPACE(ch) \
  ((ch) == ' ' || IS_PS_NEWLINE(ch) || (ch) == '\t' || (ch) == '\f' || (ch) == '\0')
...
#define IS_PS_XDIGIT(ch) \
  (IS_PS_DIGIT(ch) || ((ch) >= 'A' && (ch) <= 'F') || ((ch) >= 'a' && (ch) <= 'f'))
