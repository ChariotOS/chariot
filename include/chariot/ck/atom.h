...
namespace ck {
  ...
  struct atom {
   ...
   private:
    ...
    static constexpr int _S_min_alignment = (sizeof(T) & (sizeof(T) - 1)) || sizeof(T) > 16
                                                ? 0
                                                : sizeof(T);

    static constexpr int _S_alignment = _S_min_alignment > alignof(T) ? _S_min_alignment
                                                                      : alignof(T);

    alignas(_S_alignment) T _M_i;

    static_assert(__is_trivially_copyable(T), "std::atomic requires a trivially copyable type");
   ...
   public:
    ...
    static constexpr bool is_always_lock_free = __atomic_always_lock_free(sizeof(_M_i), 0);
#endif
    ...
    inline bool compare_exchange_weak(T& __e, T __i, memory_order __s, memory_order __f) noexcept {
