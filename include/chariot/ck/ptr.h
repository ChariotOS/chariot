...
namespace ck {
  ...
  class box {
   ...
   public:
    ...
    explicit box(T* raw_resource) noexcept : m_ptr(move(raw_resource)) {}
    box(nullptr_t) : m_ptr(nullptr) {}

    box() noexcept : m_ptr(nullptr) {}
   ...
   public:
    ...
    // safely moved.
    box(box<T>&& move) noexcept { move.swap(*this); }
    ...
    // swaps the resources
    void swap(box<T>& resource_ptr) noexcept { ::swap(m_ptr, resource_ptr.m_ptr); }
  ...
  ck::box<T> make_box(Args&&... args) {
    return ck::box<T>(new T(::forward<Args>(args)...));
  }
  ...
  class lazy_box {
   ...
   public:
    ...
    // constructed the unique_ptr will own the resource. std::move is used because
    // it is used to indicate that an object may be moved from other resource.
    explicit lazy_box(T* raw_resource) noexcept : m_ptr(move(raw_resource)) {}
    lazy_box(nullptr_t) : m_ptr(nullptr) {}

    lazy_box() noexcept : m_ptr(nullptr) {}
   ...
   public:
    // Allows move-semantics. While the unique_ptr cannot be copied it can be
    // safely moved.
    lazy_box(lazy_box<T>&& move) noexcept { move.swap(*this); }
    // ptr = std::move(resource)
    ...
    // swaps the resources
    void swap(lazy_box<T>& resource_ptr) noexcept { ::swap(m_ptr, resource_ptr.m_ptr); }
    // replaces the resource. the old one is destroyed and a new one will take
    // it's place.
    void reset(T* resource_ptr) noexcept(false) {
      // ensure a invalid resource is not passed or program will be terminated
...
// printf("ck::ref operator new called directly in userspace!\n");
  ...
  class ref {
   public:
    ...
    template <typename U>
    ref(const ck::box<U>&) = delete;
    template <typename U>
    ref& operator=(const ck::box<U>&) = delete;
