...
#if PRINTPKT_SUPPORT
static int eap_printpkt(const u_char *inp, int inlen, void (*)(void *arg, const char *fmt, ...),
                        void *arg);
...
void eap_authwithpeer(ppp_pcb *pcb, const char *localname) {
   ...
   */
  if (pcb->settings.eap_req_time > 0) TIMEOUT(eap_client_timeout, pcb, pcb->settings.eap_req_time);
}
...
static bool pncrypt_setkey(int timeoffs) {
  ...
  SHA1Final(dig, &ctxt);
  /* FIXME: if we want to do SRP, we need to find a way to pass the PolarSSL des_context instead of
   * using static memory */
    ...
    case eapIdentify:
      ...
      if (pcb->eap.es_server.ea_peerlen > SRP_PSEUDO_LEN &&
        ...
        for (i = 0; i < 5; i++) {
          ...
          toffs -= 86400;
          /* FIXME: if we want to do SRP, we need to find a way to pass the PolarSSL des_context
           * instead of using static memory */
        ...
        if (plen % 8 == 0 && i < 5) {
          ...
          ppp_dbglog("decoded pseudonym to \"%.*q\"", pcb->eap.es_server.ea_peerlen,
      ...
      if (get_srp_secret(pcb->eap.es_unit, pcb->eap.es_server.ea_peer, pcb->eap.es_server.ea_name,
                         (char *)secbuf, 1) != 0) {
...
static void srp_lwrechallenge(void *arg) {
  ...
  if (pcb->eap.es_server.ea_state != eapOpen || pcb->eap.es_server.ea_type != EAPT_SRP) return;
 ...
 */
static void eap_send_response(ppp_pcb *pcb, u_char id, u_char typenum, const u_char *str,
                              int lenstr) {
  struct pbuf *p;
  u_char *outp;
  int msglen;
 ...
 */
static void eap_chap_response(ppp_pcb *pcb, u_char id, u_char *hash, const char *name,
                              int namelen) {
  struct pbuf *p;
  u_char *outp;
  int msglen;
...
static void eap_response(ppp_pcb *pcb, u_char *inp, int id, int len) {
  ...
  switch (typenum) {
    ...
    case EAPT_MD5CHAP:
      ...
      eap_figure_next_state(pcb, 0);
      if (pcb->eap.es_rechallenge != 0) TIMEOUT(eap_rechallenge, pcb, pcb->eap.es_rechallenge);
      break;
...
static void eap_success(ppp_pcb *pcb, u_char *inp, int id, int len) {
  ...
  if (pcb->eap.es_client.ea_state != eapOpen && !eap_client_active(pcb)) {
    ppp_dbglog("EAP unexpected success message in state %s (%d)",
               eap_state_name(pcb->eap.es_client.ea_state), pcb->eap.es_client.ea_state);
    return;
  }
...
static const char *const eap_typenames[] = {
    "Identity", "Notification", "Nak",          "MD5-Challenge", "OTP",   "Generic-Token",
    NULL,       NULL,           "RSA",          "DSS",           "KEA",   "KEA-Validate",
    "TLS",      "Defender",     "Windows 2000", "Arcot",         "Cisco", "Nokia",
    "SRP"};

static int eap_printpkt(const u_char *inp, int inlen, void (*printer)(void *, const char *, ...),
                        void *arg) {
