...
static void update_should_order_children(struct flex_item *item) {
  if (item->order != 0 && item->parent != NULL) {
    item->parent->should_order_children = true;
  }
}

static void item_property_changed(struct flex_item *item, flex_property property) {
...
struct flex_item *flex_item_new(void) {
  struct flex_item *item = (struct flex_item *)calloc(1, sizeof(struct flex_item));
  assert(item != NULL);
...
static void grow_if_needed(struct flex_item *item) {
...
static void child_set(struct flex_item *item, struct flex_item *child, unsigned int index) {
...
#define LAYOUT_CHILD_AT(item, i) \
  (item->children.ary[(layout->ordered_indices != NULL ? layout->ordered_indices[i] : i)])
...
static flex_align child_align(struct flex_item *child, struct flex_item *parent) {
...
static void layout_items(struct flex_item *item, unsigned int child_begin, unsigned int child_end,
