...
void crypto::aes::expand_key(u8 *key, u32 key_len, u8 *expkey)
{
...
void crypto::aes::encrypt(u32 key_len, u8 *in, u8 *expkey, u8 *out)
{
...
void crypto::aes::decrypt(u32 key_len, u8 *in, u8 *expkey, u8 *out)
{
	u8 state[AES_STATECOLS * 4];
...
void crypto::aes::apply_cbc_chain_data(u8 *cbc_chain_data, u8 *src, u8 *dst)
{
	int i;
...
void crypto::aes::cbc_encrypt_blocks(u32 key_len, u8 *key_exp, u8 *iv, u8 *src, u8 *dst,
			    u32 num_aes_blocks)
{
	...
	for (i = 0; i < num_aes_blocks; i++) {
		...
		crypto::aes::apply_cbc_chain_data(cbc_chain_data, src, tmp_data);

		/* Encrypt the AES block */
		crypto::aes::encrypt(key_len, tmp_data, key_exp, dst);
...
void crypto::aes::cbc_decrypt_blocks(u32 key_len, u8 *key_exp, u8 *iv, u8 *src, u8 *dst,
			    u32 num_aes_blocks)
{
	...
	for (i = 0; i < num_aes_blocks; i++) {
		...
		crypto::aes::decrypt(key_len, src, key_exp, tmp_data);

		/* Apply the chain data */
		crypto::aes::apply_cbc_chain_data(cbc_chain_data, tmp_data, dst);
