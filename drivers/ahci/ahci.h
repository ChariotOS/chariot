...
namespace ahci {
  ...
  /**
   * The structure that bar5 of the PCI device points to. Typically considered the
   * ABAR
   */
  struct hba_mem {
   ...
   *
   * AHCI uses a FIS (or many) to transmit data between devices. The internal
   * structure of a FIS depends on the type. This namespace defines structures for
   * each FIS type needed and the enum that identifies them.
   */
  namespace fis {
    /**
     ...
     * IDE registers such as command, LBA, device, feature, count and control. An
     * ATA command is constructed in this structure and issued to the device. All
     * reserved fields in an FIS should be cleared to zero.
     */
    struct reg_h2d {
      // DWORD 0
    ...
    /**
     * A device to host register FIS is used by the device to notify the host that
     * some ATA register has changed. It contains the updated task files such as
     * status, error and other registers.
     */
    struct reg_d2h {
      // DWORD 0
    ...
    /*
     * This FIS is used by the device to tell the host that it’s about to send or
     * ready to receive a PIO data payload.
     */
    struct pio_setup {
      // DWORD 0
  ...
  struct disk {
    ...
    // read `count` sectors into dst
    bool read(uint64_t sector, uint32_t count, void *dst);

    void stop_cmd(void);
    void start_cmd(void);
  };

  int /* minor */ register_disk(struct ahci::disk *);
