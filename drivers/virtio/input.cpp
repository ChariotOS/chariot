...
bool virtio_mmio_input::initialize(const struct virtio_config &cfg) {
  ...
  printk(KERN_INFO "[virtio input] features: %32llb\n", features);
...

  {
    ...
    printk(KERN_INFO "[virtio input] Device Name: \'%s\'\n", s.get());
  }

  {
    ...
    ck::string s(config().u.string, config().size);
    printk(KERN_INFO "[virtio input] Device Serial Number: \'%s\'\n", s.get());
  }
...

  {
    ...
    printk(KERN_INFO "[virtio input] Bus Type: %d\n", config().u.ids.bustype);
    printk(KERN_INFO "[virtio input] Vendor:   %04x\n", config().u.ids.vendor);
    printk(KERN_INFO "[virtio input] Product:  %d\n", config().u.ids.product);
    printk(KERN_INFO "[virtio input] Version:  %d\n", config().u.ids.version);
  }
...
void virtio_mmio_input::irq(int ring_index, virtio::virtq_used_elem *) {
  printk("irq in %d\n", ring_index);
}
