...
bool virtio_mmio_gpu::initialize(const struct virtio_config &config) {
  ...
  if (int err = allocate_2d_resource(display_resource_id, pmode.r.width, pmode.r.height);
      err != 0) {
    return false;
  }
  ...
  if (int err = set_scanout(pmode_id, display_resource_id, pmode.r.width, pmode.r.height);
      err != 0) {
    return false;
  }
...
int virtio_mmio_gpu::flush_resource(int resource_id, uint32_t width, uint32_t height) {
  int err = 0;
  scoped_lock l(lock);
