...
int WI_drawNum(int x, int y, int n, int digits) {
  ...
  if (neg) V_DrawPatch(x -= 8, y, wiminus);
...
void WI_drawTime(int x, int y, int t) {
  ...
  if (t <= 61 * 59) {
    ...
    do {
      ...
      // draw
      if (div == 60 || t / div) V_DrawPatch(x, y, colon);
...
void WI_drawNetgameStats(void) {
  ...
  if (dofrags) V_DrawPatch(NG_STATSX + 4 * NG_SPACINGX - SHORT(frags->width), NG_STATSY, frags);
  ...
  for (i = 0; i < MAXPLAYERS; i++) {
    ...
    if (i == me) V_DrawPatch(x - SHORT(p[i]->width), y, star);
