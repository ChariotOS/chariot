...
void V_DrawPatch(int x, int y, patch_t *patch) {
  ...
  if (x < 0 || x + SHORT(patch->width) > SCREENWIDTH || y < 0 ||
    ...
    I_Error("Bad V_DrawPatch x=%i y=%i patch.width=%i patch.height=%i topoffset=%i leftoffset=%i",
            x, y, patch->width, patch->height, patch->topoffset, patch->leftoffset);
  }
#endif
...
void V_DrawMouseSpeedBox(int speed) {
  ...
  if (linelen < redline_x) {
    ...
    V_DrawHorizLine(box_x + redline_x, box_y + MOUSE_SPEED_BOX_HEIGHT / 2, linelen - redline_x,
                    yellow);
  }
