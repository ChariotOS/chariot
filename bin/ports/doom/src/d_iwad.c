...
static void CheckCollectorsEdition(void) {
  ...
  for (i = 0; i < arrlen(collectors_edition_subdirs); ++i) {
    subpath = M_StringJoin(install_path, DIR_SEPARATOR_S, collectors_edition_subdirs[i], NULL);
...
static boolean DirIsFile(char *path, char *filename) {
  ...
  return path_len >= filename_len + 1 && path[path_len - filename_len - 1] == DIR_SEPARATOR &&
