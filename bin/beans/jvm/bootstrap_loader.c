...
cst_new_node (cst_type_t type, const char * str_repr)
{
...
static inline void
cst_add_child (struct cst_node * root, struct cst_node * child)
{
    root->children[root->num_children++] = child;
}

static void
cst_push (struct cst_node * root, cst_type_t type, const char * str_repr)
{
 ...
 */
static void
parse_field_desc (char ** desc, struct cst_node * root, java_class_t * cls)
{
    switch (**desc) {
        case 'L':  {
...
static void
parse_return_desc (char ** desc, struct cst_node * root, java_class_t * cls)
{
 ...
 */
static struct cst_node * 
parse_method_desc (char ** desc, java_class_t * cls)
{
 ...
 */
static struct cst_node * 
parse_desc (char * desc, java_class_t * cls)
{
    if (*desc == '(') {
        desc++;
...
static char *
construct_method_sig (char ** args, char * ret_type, int nargs)
{
...
static struct cst_node *
cst_ret_type (struct cst_node * root)
{
    if (root->type != CST_METHOD)
        return NULL;

    for (int i = 0; i < root->num_children; i++) {
        if (root->children[i]->type == CST_RET)
...
static char * 
method_ret_type (struct cst_node * ret)
{
    if (!ret)
        return NULL;
...
static void
cst_args (struct cst_node * root, int * nargs, struct cst_node ** args)
{
    for (int i = 0; i < root->num_children; i++) {
...
static void
construct_field_sig (struct cst_node * root, char * buf)
{
...
static char ** 
method_args (struct cst_node ** args, int n)
{
    char ** ret = NULL;
...
static void
resolve_field_strings (field_info_t * fi, java_class_t * cls)
{
    char buf[MAX_FIELD_LEN];
    memset(buf, 0, MAX_FIELD_LEN);
...
static void
resolve_method_strings (method_info_t * mi, java_class_t * cls)
{
...
static FILE * 
open_in_dir (const char * dirname, const char * fname, const char * mode)
{
	printf("dirname=%s\n", dirname);
    ...
    while ((de = readdir(d))) {
        ...
        if (!S_ISDIR(statbuf.st_mode)) {
            if (strncmp(de->d_name, fname, NAME_MAX) == 0) {
                f = fopen(fname, mode);
                if (f)
                    goto out;
            }
...
static FILE *
hb_open (const char * fname, const char * mode)
{
    FILE * f = NULL;
...
static inline bool
is_lvar_tbl_attr (u2 name_idx, java_class_t * cls)
{
	return strncmp(hb_get_const_str(name_idx, cls), "LocalVariableTable", 18) == 0;
}

static inline bool
is_line_tbl_attr (u2 name_idx, java_class_t * cls)
{
    return strncmp(hb_get_const_str(name_idx, cls), "LineNumberTable", 15) == 0;
}

static inline bool
is_source_file_attr (u2 name_idx, java_class_t * cls)
{
...
parse_method_code (u1 * clb, method_info_t * m, java_class_t * cls)
{
	...
	m->code_attr->attr_count = get_u2(clb); clb+=2;
    ...
    for (i = 0; i < m->code_attr->attr_count; i++) {
        ...
        u2 name_idx = get_u2(clb); clb+=2;
        u4 len      = get_u4(clb); clb+=4;
        ...
        if (is_line_tbl_attr(name_idx, cls)) {
            m->line_info = attr;
            u2 nents = get_u2(clb); clb += 2;
            attr->line_num_tbl.line_tbl_len = nents;

            for (int j = 0; j < nents; j++) {
                u2 start_pc = get_u2(clb); clb += 2;
                u2 line_num = get_u2(clb); clb += 2;
            ...
            for (int j = 0; j < nents; j++) {
                u2 start_pc = get_u2(clb); clb += 2;
                u2 len = get_u2(clb); clb += 2;
                u2 nidx = get_u2(clb); clb += 2;
                u2 didx = get_u2(clb); clb += 2;
                u2 idx = get_u2(clb); clb += 2;
...
static int
parse_attr (u1 * clb, java_class_t * cls, u4 idx, u4 * skip)
{
    ...
    *skip += 6;
    ...
    if (is_source_file_attr(name_idx, cls)) {
        ainfo->src_file.srcfile_idx = get_u2(clb); clb += 2;
...
int
hb_read_source_file (java_class_t * cls)
{
    int n = 0;
    size_t bytes = 0;
