...
print_pc_update (jthread_t * t)
{
    ...
    HB_INFO_NOBRK("    %-8s: " BLUE("%s"), "SOURCE",
            hb_get_srcline_from_pc(t->cur_frame->pc, t->cur_frame));
...
static int
cmd_help (jthread_t * t, char * args)
{
	print_usage();
	return 0;
}
...
// TODO
static int
cmd_print (jthread_t * t, char * args)
{
    // search in fields,
    // locals
...
static bool 
args_match (struct bp_method_desc * a, struct bp_method_desc * b)
{
    if (a->nargs != b->nargs)
        return false;
...
static bool
method_matches_desc (method_info_t * mi, java_class_t * cls, struct bp_method_desc * desc)
{
    if (desc->nargs != mi->nargs) 
        return false;

    for (int i = 0; i < mi->nargs; i++) {
        if (strcmp(mi->args[i], desc->args[i]) != 0)
            return false;
    }
...
is_valid_bp (jthread_t * t)
{
    for (int i = 0; i < MAX_BPS; i++) {
        ...
        if (bpi.bps[i].type == BP_TYPE_LINENUM)
            continue;

        // do we match by method?
        if (bpi.bps[i].type == BP_TYPE_METHOD) {
            if (strcmp(hb_get_class_name(t->cur_frame->cls), bpi.bps[i].method.class_name) != 0)
                return -1;
            if (strcmp(t->cur_frame->minfo->name, bpi.bps[i].method.method_name) != 0)
                return -1;
            if (method_matches_desc(t->cur_frame->minfo, t->cur_frame->cls, &(bpi.bps[i].method)))
                return i;
        }
    }
...
static bool
bp_exists (struct bp_method_desc * desc)
{
    for (int i = 0; i < MAX_BPS; i++) {
        if (!bpi.bps[i].valid) 
            continue;

        if (strcmp(desc->class_name, bpi.bps[i].method.class_name) != 0)
            continue; 

        if (strcmp(desc->method_name, bpi.bps[i].method.method_name) != 0)
            continue;

        if (!args_match(desc, &(bpi.bps[i].method)))
            continue;
...
static void 
check_res_bump_pc (int res, jthread_t * t)
{
    if (res == -1) {
...
bp_rm (int id)
{
    if (id >= MAX_BPS || bpi.bps[id].valid == false)
        return -1;
...
static int
cmd_step (jthread_t * t, char * args)
{
    int bp_hit;
    int res     = 0;
 ...
 */
static int
cmd_cont (jthread_t * t, char * args)
{
    int bp_hit;
    int res     = 0;
...
static int
cmd_stepi (jthread_t * t, char * args)
{
    int bp_hit;
    int res     = 0;

    if (((bp_hit = is_valid_bp(t)) == -1) && !sigint_recvd) 
        res = hb_exec_one(t);
...
static int
cmd_list (jthread_t * t, char * args)
{
    stack_frame_t * frame = t->cur_frame;
...
static int
cmd_classes (jthread_t * t, char * args)
{
...
static int
cmd_class (jthread_t * t, char * args)
{
    HB_ERR("%s not implemented", __func__);
    return 0;
}
...
static int
cmd_locals (jthread_t * t, char * args)
{
    hb_dump_locals();
    return 0;
}
...
static int
cmd_methods (jthread_t * t, char * args)
{
    ...
    while (cls) {
        for (int i = 0; i < hb_get_method_count(cls); i++) {
            ...
            HB_INFO(CYAN("%-20s") " " GREEN("%s %s"), hb_get_class_name(cls), 
                    cls->methods[i].name, 
                    cls->methods[i].desc);
        }
...
static int
cmd_fields (jthread_t * t, char * args)
{
    java_class_t * cls = NULL;
    ...
    for (int i = 0; i < hb_get_field_count(cls); i++) {
        ...
        HB_INFO("  %s %s", cls->fields[i].desc_str,
                           cls->fields[i].name_str);
    }
...
static int
get_method_matches (method_info_t ** matches, java_class_t * cls, char * method_name)
{
    int nmatches = 0;
    char buf[128];
    ...
    for (int i = 0; i < hb_get_method_count(cls); i++) {
        method_info_t * mi = &cls->methods[i];
        if (strcmp(mname, hb_get_method_name(mi)) == 0) 
            matches[nmatches++] = mi;
    }
...
static int
insert_bp (struct bp_method_desc * desc)
{
    int id = -1;
...
bp_list (void)
{
    for (int i = 0; i < MAX_BPS; i++) {
        if (bpi.bps[i].valid == true) {
            HB_INFO("[" BRCYAN("%3d") "]  " GREEN("%s.") YELLOW("%s %s"), 
                    i, 
                    bpi.bps[i].method.class_name,
                    bpi.bps[i].method.method_name,
...
static int
cmd_stop (jthread_t * t, char * args)
{
...
static int
cmd_throw (jthread_t * t, char * args)
{
...
static int
cmd_where (jthread_t * t, char * args)
{
    stack_frame_t * frame = t->cur_frame;
    int frame_num = 1;
...
static int
cmd_clear (jthread_t * t, char * args)
{
    clear_bps();
    return 0;
}
...
static int
cmd_bp_list (jthread_t * t, char * args)
{
    bp_list();
    return 0;
}
...
static int
cmd_bp_rm (jthread_t * t, char * args)
{
    char * tok = next_token(&args);
    int id = atoi(tok);
    if (bp_rm(id) < 0) 
        HB_INFO("No such breakpoint");
...
static noreturn int
cmd_quit (jthread_t * t, char * args)
{
...
static const command_descriptor_t commands[] = {
    ...
    {SPELLINGS("list", "ls"),
        "",
        "Show source listing at current PC",
        cmd_list},

	{SPELLINGS("where", "w"),
		"",
		"Dumps the stack of the current thread, with PC info",
		cmd_where},

	{SPELLINGS("print", "pr"),
		"<variable>",
		"Prints a primitive value, an object's instance fields, or if it's a class, its static fields",
		cmd_print},
	...
	{SPELLINGS("throw", "t"),
		"<Class ID>",
		"Raises an Exception of named class",
		cmd_throw},

    {SPELLINGS("instr", "bc"),
        "",
        "Disassembles the current bytecode instruction (if applicable)",
        cmd_bc},
    ...
    {SPELLINGS("class", "cl"),
        "<class ID>",
        "Show details of named class",
        cmd_class},

    {SPELLINGS("classes", "cls"),
        "",
    ...
    {SPELLINGS("stop"),
        ...
        "in <Class ID>.<method>[(argument_type,...)]", 
...
static int
handle_cmd (jthread_t * t, char * line)
{
...
void
run_shell (jthread_t * t, bool interactive)
{
	printf("TODO: the shell doesn't work lol\n");
#if 0
