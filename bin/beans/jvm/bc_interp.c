...
#define GET_4B_IDX(bcp) ((u4)(bcp)[1] << 24u | (u4)(bcp)[2] << 16u | \
			 (u4)(bcp)[3] << 8u | (u4)(bcp)[0])
...
{	
  ...
  if ((v.tag & expected_tag_flags) == 0 && expected_tag_flags != TAG_ANY) {
    HB_ERR("%s: got wrong variable tag, expected '%s', got '%s'",
        __func__,
        TAG_STR(expected_tag_flags),
        TAG_STR(v.tag));
  }
  */
...
handle_iand (u1 * bc, java_class_t * cls) {
	DO_LOGOP(&, int_val, u4, i4, TAG_INT);
}

static int
handle_land (u1 * bc, java_class_t * cls) {
	DO_LOGOP(&, long_val, u8, i8, TAG_LONG);
}
...
handle_getfield (u1 * bc, java_class_t * cls) {
    ...
    HB_ERR("%s: Cannot resolve field, index out of bounds. "
           "Got idx = %d except length = %d", __func__, val_offset, obj->field_count);
    return -1;
	}
